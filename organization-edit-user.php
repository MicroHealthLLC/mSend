<?php
/**
 * Show the form to edit an existing Organization.
 *
 * @package		ProjectSend
 * @subpackage	Groups
 *
...
$page_title = __('Edit Users Organization','cftp_admin');
					...
					<?php
						if (isset($edit_response)) {
							...
							switch ($edit_response['query']) {
								case 1:
									$msg = __('Organization edited correctly.','cftp_admin');
						 ...
						 */
							...
							if ($page_status === 0) {
								$msg = __('No Organization was selected.','cftp_admin');
								echo system_message('error',$msg);
								echo '<p>'.$direct_access_error.'</p>';
							}
							else if ($page_status === 2) {
								$msg = __('There is no Organization with that ID number.','cftp_admin');
								echo system_message('error',$msg);
								echo '<p>'.$direct_access_error.'</p>';
							}
							else {
								 ...
								 * Include the form.
								 */
								$organization_form_type = 'edit_organization';
								include('organization-form-user.php');
							}
						}
					?>
